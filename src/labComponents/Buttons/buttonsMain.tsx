    ...
    <>
      {buttons.map((button) => {
        if (button.type === 'type1') {
          ...
          return <Button2 unique_id = {button.unique_id} key={button.id} position={button.position} rotation={button.rotation} />;
        } else if (button.type === 'type3') {
          return <TriangleButton key={button.id} position={button.position} rotation={button.rotation} />; 
        }
        return null;
      })}
    </>
