...
const TriangleButton: React.FC<TriangleButtonProps> = ({ position, rotation = [Math.PI / 2, 0, 0], scale = [1, 1, 1] }) => {
    ...
    useFrame(() => {
        if (groupRef.current) {
            if (isMovingBack) {
                const newPosZ = THREE.MathUtils.lerp(groupRef.current.position.z, position[2] + 0.5, 0.1); 
                ...
                const newPosZ = THREE.MathUtils.lerp(groupRef.current.position.z, position[2], 0.1);
                groupRef.current.position.set(position[0], position[1], newPosZ);
